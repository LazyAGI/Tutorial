...
-   **第21课时：长上下文能力增强**
    ...
    -   **LazyLLM 评测实战**：NIAH 大海捞针测试可视化（位置-长度热力图），LongBench 多任务评测集，以 run_squad_ppl.py 监控 PPL 随序列长度的衰减曲线。
-   **第22课时：结构化输出与格式对齐**
    -   **数据构建流水线**：Schema 设计（JSON Schema/Pydantic 字段定义）→ 逆向合成（先生成目标 JSON 再反推自然语言描述）→ 自动化校验（jsonschema 验证）→ 负样本构建（字段缺失/类型错误/格式违规样本）。
    -   **TypeScript 风格提示工程**：以类型声明风格书写 Schema 约束，少样本 Schema 示例注入，边界条件与异常处理提示策略。
    -   **语法引导解码**：基于 Trie 树的 Token 级约束（outlines/lm-format-enforcer），EBNF 文法规则，推理阶段非法 token 路径剪枝原理。
    -   **评测指标**：格式错误率、字段级精确率（每字段的值准确率）、幻觉率（LLM 编造字段值的比例）。
    -   **LazyLLM 实战**：Text2SQL 流水线（run_text2sql_pipeline.py）与中文医疗实体关系抽取 CMEIE（run_cmeie.py）完整实验。
