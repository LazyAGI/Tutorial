...
| [API-Bank](https://huggingface.co/papers/2304.08244) | 完整评估系统，工具使用三层级 | 73种API | 对话系统API调用评估 | 首个具备评估基准 |
| [ToolAlpaca](https://github.com/tangqiaoyu/ToolAlpaca) | 指令微调数据，585个工具 | 52k样本 | 指令跟随工具调用 | 低资源模型适配 |
| [ToolQA](https://github.com/night-chen/ToolQA) | 问答式工具调用，复杂推理 | 10k+样本 | 多跳推理工具使用 | 结合推理和工具调用 |
| [RESTBench](https://github.com/Yifan-Song793/RestGPT) | RESTful API调用，参数生成 | 8k+样本 | Web服务集成 | 标准化API调用格式 |

**🧠 规划能力 (Planning) 经典数据集**

| 数据集名称 | 核心特点 | 规模 | 应用场景 | 关键贡献 |
| :--- | :--- | :--- | :--- | :--- |
| [AgentInstruct](https://github.com/wang-research-lab/agentinstruct) | GPT-4合成，操作系统/数据库交互 | 1.8k+核心种子 | 操作系统控制、文件处理 | 高质量合成数据 |
| [WebArena](https://webarena.dev/) | 真实Web环境，完整任务规划 | 812个任务 | Web自动化、电商操作 | 端到端Web代理 |
| [Mind2Web](https://github.com/OSU-NLP-Group/Mind2Web) | 真实网页操作，DOM元素定位 | 2k+任务 | 浏览器自动化 | Web代理基础 |
...
| [Bamboogle](https://huggingface.co/datasets/chiayewken/bamboogle) | 搜索引擎交互，查询规划 | 3.2k样本 | 信息搜索规划 | 搜索引擎代理 |
...
| 数据集名称 | 核心特点 | 规模 | 应用场景 | 关键贡献 |
| :--- | :--- | :--- | :--- | :--- |
| [OSWorld](https://os-world.github.io/) | 真实操作系统环境，视觉+执行 | 369+任务 | 跨平台 GUI 自动化 | 首个可交互真实 OS 评测 |
| [VisualWebArena](https://jykoh.com/vwa) | 网页视觉理解+操作 | 910+任务 | 网页端 GUI Agent | 视觉感知 Web 交互基准 |
| [Mind2Web](https://github.com/OSU-NLP-Group/Mind2Web) | 真实网页，DOM+视觉双模态 | 2k+任务 | 网页操作 | 端到端 Web Agent 基础 |
| [AITW](https://github.com/google-research/google-research/tree/master/android_in_the_wild) | Android 真实设备操作 | 30k+步骤 | 移动端 GUI 自动化 | 大规模 Android 操作数据 |
| [GUI Odyssey](https://github.com/OpenGVLab/GUI-Odyssey) | 跨应用导航任务 | 7.5k+任务 | 跨应用任务执行 | 多应用 GUI 导航 |
...
除上述学术趋势外，2024-2025 年产业界正推动 Agent 领域的**标准化与生态化**，呈现出以下关键发展方向：
...
* **数据构建挑战**：
    * 需要大量**屏幕截图-操作对**数据（Screenshot-Action Pairs）。
    * 操作需要精确到**像素级坐标**或**元素选择器**。
    * 必须包含**错误恢复路径**（如弹窗拦截、页面加载失败）。
...
2. **大规模代码库理解**：
   * 需要构建**代码库级上下文数据**：如何在整个项目中定位相关文件、理解模块间依赖。
   * 数据格式通常包含`repo_structure`、`relevant_files`、`cross_file_refs`等字段。

3. **人机协作轨迹**：
   * 真实场景下，开发者会纠正 Agent、提供方向。这些**人类反馈信号**（如"这里应该用递归而非循环"）是高质量训练数据。

4. **工具调用原子化**：
   * `read_file`、`edit_file`、`run_test`、`git_commit`等工具需要精确参数（文件路径、行号范围）。
   * 数据需包含从自然语言意图到精确工具调用的映射。

**代表数据集与基准：**

| 数据集/基准 | 核心特点 | 应用场景 |
| :--- | :--- | :--- |
| [SWE-bench](https://www.swebench.com/) | 真实 GitHub Issue → PR 修复 | 端到端代码修复能力评估 |
| [HumanEval](https://github.com/openai/human-eval) | 函数级编程题 | 代码生成基础能力 |
| [MBPP](https://github.com/google-research/google-research/tree/master/mbpp) | Python 编程问题 | 入门级代码合成 |
| [ExecRepoBench](https://execrepobench.github.io/) | 仓库级代码执行任务 | 大规模代码库理解 |
...
复杂任务正越来越多地由**多 Agent 协作系统**完成，而非单体 Agent。
...
OpenAI Operator、Devin、Manus 等产品级 Agent 正在构建**真实世界的数据飞轮**：
...
我们正处于从”被动知识引擎（Chatbot）”向”主动行动者（Agent）”跨越的关键历史节点。掌握**工具定义的标准化**（MCP 协议）、**规划数据的图结构化**（ToT/GoT）、**环境模拟的自动化**（Simia 范式）以及**多 Agent 协作机制**，是通往通用人工智能（AGI）的必经之路。
...
OpenAI Operator、Devin、Manus 等产品级 Agent 的数据飞轮，预示着**高质量交互数据**将成为下一代 Agent 竞争的核心壁垒。而 MCP、A2A 等协议标准的出现，则标志着 Agent 生态从”各自为战”走向”互联互通”。

未来的 Agent，将不再仅仅是回答问题的助手，而是能够真正理解意图、拆解目标、在动态环境中生存、与其他 Agent 协作、自主开发软件并持续进化的**自主智能实体**。
...
> ```bash
> python run_tooluse_pipeline.py --lazyllm-path /path/to/lazyllm
> ```
...
- `./assets/agent.jpeg`：[知乎专栏《智能代理Agent：AI 智能体》](https://zhuanlan.zhihu.com/p/659386520)
- `./assets/distllstep.png`：[ArXiv 论文《Distilling Step-by-Step! Outperforming Larger Language Models with Less Training Data and Smaller Model Sizes》](https://arxiv.org/abs/2305.02301)
