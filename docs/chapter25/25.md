...
![alt text](./assets/3.png)（来源：Agentic RAG: Turning Your RAG Pipeline Into a Thinking System）
...
在介绍完相关功能后，我们使用 LazyLLM 实际构建一个基础的 RAG 知识库，完整代码见 [rag_kb_build.py](from-data-to-llm/docs/chapter25/code/rag_kb_build.py)。整体流程为：**收集源文件 → 分块流水线 →（可选）LLM 清洗 → 导出为知识库目录 → 可选检索演示**。
