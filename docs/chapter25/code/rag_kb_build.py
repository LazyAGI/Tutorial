...
def run_chunk_pipeline(text_paths: list, output_dir: Path, chunk_size: int, chunk_overlap: int) -> list:
...
def run_clean_pipeline(chunk_paths: list, output_dir: Path, llm, lang: str = "zh") -> list:
...
def export_chunks_to_kb_dir(results: list, use_cleaned: bool, kb_content_dir: Path) -> None:
    ...
    for item in results:
        ...
        if not chunks:
            continue
        base_name = Path(path).stem.replace("_chunk", "").replace("_cleaned", "")
        out_file = kb_content_dir / f"{base_name}.txt"
        out_file.write_text("\n\n".join(chunks), encoding="utf-8")


def main():
    parser = argparse.ArgumentParser(description="基于 LazyLLM KBC 构建 RAG 知识库")
    parser.add_argument(
        "--sources_dir",
        type=str,
        default=None,
    ...
    if not args.sources_dir.is_dir():
        ...
        print("请先创建并在其中放入 .txt / .md 文件，或使用默认 rag_kb/sources 并放入示例文件。")
        sys.exit(1)
    ...
    retriever = lazyllm.Retriever(doc=doc, group_name=lazyllm.Document.CoarseChunk, similarity='bm25_chinese', topk=3)
    demo_result = retriever('玉山箭竹生长在哪里？')
    print("\n示例检索结果（首条内容摘要）:")
    if demo_result and len(demo_result) > 0:
        content = getattr(demo_result[0], 'get_content', lambda: str(demo_result[0]))()
