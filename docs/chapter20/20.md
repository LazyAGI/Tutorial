...
$$ P(\text{Codebase}) = P(f_1) \cdot P(f_2 | f_1) \cdot P(f_3 | f_1, f_2) \cdots $$
如果 $f_2$ 依赖 $f_1$，那么先学习 $f_1$ 能够显著提高预测 $f_2$ 的概率 $P(f_2 | f_1)$。
...
$$ \mathcal{L}_{\text{CLM}} = - \sum_{t} \log P(x_t | x_{<t}) $$
...
$$ P(\text{Middle} | \text{Prefix}, \text{Suffix}) $$
...
$$ \mathcal{L}_{\text{FIM}} = - \sum_{i \in \text{Middle}} \log P(x_i | \text{Prefix}, \text{Suffix}, x_{<i \in \text{Middle}}) $$
