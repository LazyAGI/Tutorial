# 第20课时：代码能力增强 (Code Ability Enhancement)
...
### 2. 数据集简介：tiny-codes
*   **数据集地址**: `https://huggingface.co/datasets/nampdn-ai/tiny-codes`
...
为了方便快速复现实验，我们提供了[一键运行脚本](./code/run.py)，整合数据下载、Pipeline处理、SFT训练、评测集推理和代码评估五个阶段。
...
2. **方式二：修改脚本中的默认配置**（编辑 `run.py` 顶部变量）：

```python
LAZYLLM_PATH = "/path/to/your/lazyllm"        # lazyllm库路径
PIPELINE_MODEL = "/path/to/pipeline/model"    # 数据增强用模型
SFT_MODEL = "/path/to/sft/base/model"         # SFT训练基础模型
```
...
# 指定模型路径运行
python run.py --sft-model /model/qwen --pipeline-model /model/pipeline
...
| 步骤 | 功能 | 输出 |
|------|------|------|
| 1. 下载数据 | 从HuggingFace下载tiny-codes，筛选6000条Python代码 | `data/train_python.json` (5000条)<br>`data/eval_python.json` (1000条) |
| 2. Pipeline处理 | 使用lazyllm的codegen pipeline进行数据增强和质量筛选 | `data/codegen.json` |
| 3. SFT训练 | 使用LLaMA-Factory对基础模型进行微调 | `models/checkpoint/` |
| 4. 评测集推理 | 使用训练好的模型对评测集进行推理 | `output/inference_results.json` |
| 5. 代码评估 | 用Docker沙箱执行生成的代码并统计通过率 | `output/evaluation_report.csv` |
...
- **Pipeline采样**：默认处理5000条训练数据，可在脚本中修改 `target_count` 调整数量
- **评估并行**：评估阶段使用5个并发线程，Docker内存限制128MB，超时10秒
- **依赖要求**：需预先安装 `lazyllm`, `datasets`, 以及Docker环境
...
下图展示了评测阶段的原始日志摘要与最终统计结果：

![PPL评测日志](./assets/re_ppl_eval.png)

![PPL评测结果](./assets/re_ppl_result.png)

完整运行日志见：[ppl.log](./assets/ppl.log)

完整日志录制如下:

<div style="text-align:center; margin:20px 0;">
  <video controls style="width:900px; max-width:100%; height:auto; border:1px solid #ccc; border-radius:8px; box-shadow:0 4px 8px rgba(0,0,0,0.1);">
    <source src="./assets/re_ppl.mp4" type="video/quicktime" />
