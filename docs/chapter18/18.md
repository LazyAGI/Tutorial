...
为便于快速复现DPO安全对齐实验，我们提供了一键运行脚本 `run.sh`，整合数据准备、Preference Pipeline处理、DPO训练和安全性评估四个阶段。
...
| 4. 安全性评估 | 用LLM-as-Judge评估模型安全性 | `output/safety_evaluation.json` |

### 注意事项

- **数据缓存**：每个步骤会检查输出文件是否存在，存在则跳过，如需重新执行请删除对应文件
- **Pipeline采样**：默认处理前2000条训练数据，可在脚本第102行修改 `data[:2000]` 调整数量
- **评估样本**：默认评估100条测试样本，可在脚本第244行修改 `test_data[:100]` 调整数量
