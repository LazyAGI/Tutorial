...
> **注意**：DPO和pipeline原数据均取5k。

#### 2.3 数据集的结构

| 字段名 (Key) | 说明 | 示例内容 |
| :--- | :--- | :--- |
| **instruction** | 进化后的复杂指令文本 | "请编写一个具备错误处理机制的 Python 异步爬虫..." |
| **input** | 可选的上下文输入 | (通常为空，因为指令本身已包含足够背景) |
| **output** | 模型生成的参考答案 | "import asyncio\n..." |
...
[
  ...
  { "content": "帮我写一封请假邮件，理由是家里有事需要处理" },
  { "content": "请描述一下如何制作一道简单的家常菜，比如番茄炒蛋" },
  { "content": "帮我总结一下人工智能在医疗领域的主要应用场景" }
]
```
...
[
  {
    ...
    "chosen": "To assist you, please provide the sentence you are referring to...",
    "rejected": "To help identify a hidden rule related to a specific type of animal in a given sentence..."
  },
  {
    "instruction": "使用给定的公式计算文本的情感倾向...",
    "chosen": "为了计算文本的情感倾向，我们首先需要明确使用哪种方法或公式...",
...
**算子1：IntentExtractor（意图提取器）**
*   **功能**：调用大模型，将用户冗长或口语化的原始文本，提炼成核心的、结构化的意图。
*   **输入 (Input)**：`content` 字段。例如："我下周要去北京旅游，帮我做个攻略"
*   **输出 (Output)**：`intent` 字段（JSON 格式）。
*   **示例**：
    ```json
    {
      "content": "我下周要去北京旅游，帮我做个攻略",
      "intent": {"core_request": "制定北京旅游攻略", "time": "下周"}
    }
    ```

**算子2：PreferenceResponseGenerator（偏好回复生成器）**
*   **功能**：根据提取出的意图，让大模型生成 n 个不同的回复版本（代码中默认 n=3）。通过设置温度参数（temperature=1.0），确保生成的回答具有多样性。
*   **输入 (Input)**：`intent` 字段。
*   **输出 (Output)**：`responses` 字段（包含 3 个不同回答的列表）。
*   **示例**：
    ```json
    {
      "content": "我下周要去北京旅游，帮我做个攻略",
      "intent": {"core_request": "制定北京旅游攻略"},
      "responses": [
         "回复A：北京欢迎您！第一天可以去天安门...",
         "回复B：建议准备好舒适的鞋子。必去景点有...",
         "回复C：去北京玩啊，直接报个团吧。"
      ]
    }
    ```

**算子3：ResponseEvaluator（回复评测判官）**
*   **功能**：作为一个“裁判（LLM-as-a-Judge）”，对生成的多个回复进行打分。打分维度包括有用性（4分）、真实性（3分）和流畅度（3分），满分 10分。
*   **输入 (Input)**：`content`（原始指令）和 `responses`（生成的回复列表）。
*   **输出 (Output)**：`evaluation` 字段（包含对应每个回复总分的列表）。
*   **示例**：
    ```json
    {
      "content": "我下周要去北京旅游...",
      "responses": ["回复A", "回复B", "回复C"],
      "evaluation": [9.0, 7.5, 4.0]
    }
    ```
    (注：这里大模型认为回复 A 最好，得 9 分；回复 C 敷衍，只得 4 分。)

**算子4：PreferencePairConstructor（偏好对构造器）**
*   **功能**：也是流水线的最后一步，根据打分结果，把回答拆分成“被选中的（chosen）”和“被拒绝的（rejected）”配对数据，用于后续的模型微调。
*   **输入 (Input)**：`intent`（作为最终的 instruction），responses，以及对应的 evaluation 分数。
*   **输出 (Output)**：包含 `instruction`, `chosen`, `rejected` 的标准 DPO 格式字典。
*   **示例**（假设使用 `max_min` 策略）：算子会挑选得分最高的（回复 A，9 分）作为 chosen，得分最低的（回复 C，4 分）作为 rejected。最终清洗输出为：
    ```json
    {
      "instruction": "制定北京旅游攻略",
      "chosen": "回复A：北京欢迎您！第一天可以去天安门...",
      "rejected": "回复C：去北京玩啊，直接报个团吧。"
    }
    ```
...
model = lazyllm.TrainableModule(model_path,target_path='/home/mnt/huangchongjin/LLM/GRPO/dpo/ppl_threshold_checkpoint')\
    .mode('finetune')\
    .trainset('/home/mnt/huangchongjin/LLM/GRPO/dpo/dpo_data/threshold_preference_realdata.json')\
    .finetune_method((finetune.llamafactory, {
        ...
        'stage':'dpo',
        'dpo_beta': 0.1,
        # 'resume_from_checkpoint': '/home/mnt/huangchongjin/LLM/GRPO/code_sft/checkpoint',
        'save_steps': 10,
...
**2. ROUGE (Recall-Oriented Understudy for Gisting Evaluation) 系列**
