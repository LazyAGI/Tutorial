...
### 5. 进阶实战：基于 LazyLLM 与 DeepSeek-R1 蒸馏数据的 GRPO 完整训练
...
为便于快速复现DPO安全对齐实验，我们提供了[一键运行脚本](./code/run_ppl.py)，整合数据准备、Preference Pipeline处理、DPO训练、评测集推理和安全性评估五个阶段。
...
1. **修改脚本配置**（编辑 `run.sh` 顶部变量）：

```bash
LAZYLLM_PATH="/path/to/your/lazyllm"        # lazyllm库路径
PIPELINE_MODEL="/path/to/pipeline/model"    # Preference Pipeline用模型
DPO_BASE_MODEL="/path/to/dpo/base/model"    # DPO训练基础模型
JUDGE_MODEL="/path/to/judge/model"          # 安全性评估Judge模型
```
...
| 步骤 | 功能 | 输出 |
|------|------|------|
| 1. 数据准备 | 从HuggingFace下载PKU-SafeRLHF，取10000条训练+1000条测试 | `data/pku_raw.jsonl` (原始数据)<br>`data/ppl_input.json` (Pipeline输入)<br>`data/test.jsonl` (测试集) |
| 2. Pipeline处理 | 使用Preference Pipeline生成偏好对(chosen/rejected) | `data/train_ppl_dpo.json` |
| 3. DPO训练 | 使用LLaMA-Factory进行DPO训练 | `models/dpo_checkpoint/` |
| 4. 评测集推理 | 使用DPO模型对测试集进行推理 | `output/inference_results.json` |
| 5. 安全性评估 | 用LLM-as-Judge评估模型安全性 | `output/safety_evaluation.json` |
...
日志包含每个步骤的执行时间、状态信息和错误记录，便于排查问题。

![测评结果如图](./assets/re_eval_ppl.png)

[测评日志结果如下](./assets/ppl.log)

完整日志录制如下:

<div style="text-align:center; margin:20px 0;">
  <video controls style="width:900px; max-width:100%; height:auto; border:1px solid #ccc; border-radius:8px; box-shadow:0 4px 8px rgba(0,0,0,0.1);">
    <source src="./assets/dpo_ppl.mp4" type="video/quicktime" />
    Your browser does not support the video tag.
  </video>
</div>
...
- **数据缓存**：每个步骤会检查输出文件是否存在，存在则跳过，如需重新执行请删除对应文件
- **Pipeline采样**：默认处理前2000条训练数据，可在脚本第102行修改 `data[:2000]` 调整数量
- **评估样本**：默认评估全部测试样本，可在脚本中调整采样数量
- **依赖要求**：需预先安装 `lazyllm`, `datasets`，以及可用的GPU资源