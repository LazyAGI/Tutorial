...
```
指令：请用一句话概括《小王子》的故事。
输出：小王子讲述了一个孤独的小王子在宇宙中旅行并学会爱的故事。
```
...
![知识蒸馏教师学生模型示意图](./assets/kd.png)
> 教师模型与学生模型同时预测，使用损失函数使学生模型输出与教师模型对齐。
<!-- > 图片来源：https://blog.cubed.run/knowledge-distillation-the-underrated-champion-in-model-optimization-fcf5e196eb2e?gi=e83ab7bf8f9b -->
...
> [源代码](https://github.com/tianyi-lab/Cherry_LLM/blob/main/cherry_seletion/data_by_IFD_vic.py): 基于源代码，我们基于 LazyLLM 打造了一个流水线算子，用于批量处理数据
...
![WildJailbreak数据集样例截图](./assets/ds.png)
...
- **learning_rate**：学习率，控制模型参数更新的步长。本实验设置为 `1e-4`，在保证收敛速度的同时避免训练不稳定。

- **cutoff_len**：输入序列的最大长度（token 数）。超过该长度的文本将被截断。本实验设置为 `512`，在计算效率和上下文信息之间取得平衡。

- **max_samples**：用于训练的最大样本数量。当数据集较大时，可通过该参数限制参与训练的数据规模。本实验设置为 `10000`，用于控制训练时间和资源消耗。

- **val_size**：验证集所占比例。本实验设置为 `0.1`，即从训练数据中划分 10% 作为验证集，用于模型性能评估和早停判断。

- **per_device_train_batch_size**：每个设备（GPU/CPU）上的训练批大小。本实验设置为 `24`，在显存允许范围内尽可能提高吞吐量。

- **num_train_epochs**：训练轮数（epoch），表示整个训练集被完整遍历的次数。本实验设置为 `3.0`，在避免过拟合的同时保证模型充分学习数据分布。
...
![强化微调前回答示例图](./assets/ori.png)

> 强化微调

![强化微调后回答示例图](./assets/enh_sft.png)
...
[一键启动脚本](./codes/enqappl.py)

## 参考文献

[Knowledge Distillation: The Underrated Champion in Model Optimization](https://blog.cubed.run/knowledge-distillation-the-underrated-champion-in-model-optimization-fcf5e196eb2e?gi=e371e95ab299)
