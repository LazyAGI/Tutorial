...
完整代码[sft_llm](from-data-to-llm/docs/chapter24/code/sft_llm.py)
```python
import os
import lazyllm
...
```
## 5. 效果评估
为全面衡量微调后大语言模型在金融问答任务上的性能，本系统设计了一套多维度的自动评估方案。该方案综合考虑**答案准确性**、**语义相似性**和**内容忠实性**三个关键维度，分别通过 **Exact Match（精确匹配）**、**Cosine Similarity（余弦相似度）** 和 **Word Containment（词项包含度）** 三项指标进行量化评估。
...
**定义**：衡量模型生成的答案是否与标准答案在字符串级别完全一致。适用于答案形式唯一、结构固定的场景（如数值、日期、专有名词等）。

**计算公式**：

设第 \( i \) 个样本的模型输出为 \( \hat{y}_i \)，标准答案为 \( y_i \)，则其精确匹配得分 \( s_i^{\text{exact}} \) 定义为：
...
\[
\text{EM} = \frac{1}{N} \sum_{i=1}^{N} s_i^{\text{exact}}
\]
...
**定义**：基于语义嵌入向量，衡量模型输出与标准答案在语义空间中的相似程度。可容忍表达方式不同但语义等价的回答。

**计算流程**：
- 使用预训练中文嵌入模型（如 `bge-large-zh-v1.5`）将文本映射为向量；
- 计算两个向量的余弦相似度。

**公式**：

设 \( \mathbf{v}_{\hat{y}_i} \) 和 \( \mathbf{v}_{y_i} \) 分别为模型输出 \( \hat{y}_i \) 与标准答案 \( y_i \) 的嵌入向量，则余弦相似度得分为：

\[
s_i^{\text{cos}} = \frac{\mathbf{v}_{\hat{y}_i} \cdot \mathbf{v}_{y_i}}{\|\mathbf{v}_{\hat{y}_i}\| \cdot \|\mathbf{v}_{y_i}\|}
\]
...
取值范围为 \([0, 1]\)，值越接近 1 表示语义越相似。

---

#### 3. **Origin Score（原文约束得分）**

**定义**：评估模型生成的答案是否完全由输入上下文（`context`）中的词汇构成，防止模型“幻觉”（hallucination），即生成上下文未提及的信息。

**实现逻辑**：
- 对上下文 \( c_i \) 和模型输出 \( \hat{y}_i \) 进行分词（如使用 Jieba）；
- 判断 \( \hat{y}_i \) 中所有词是否均属于 \( c_i \) 的词集合。

**公式**：

设 \( W(c_i) \) 为上下文 \( c_i \) 的词集合，\( W(\hat{y}_i) \) 为模型输出的词集合，则忠实性得分定义为：

\[
s_i^{\text{orig}} = 
\begin{cases}
1, & \text{if } W(\hat{y}_i) \subseteq W(c_i) \\
0, & \text{otherwise}
\end{cases}
\]

整体忠实性比率（Origin Score）为：

\[
\text{Origin} = \frac{1}{N} \sum_{i=1}^{N} s_i^{\text{orig}}
\]

> **注意**：实际实现中可能对停用词、标点或数字做特殊处理，以提升鲁棒性。

---

### 5.2 评估流程

函数 `caculate_score(eval_set, infer_set)` 执行以下步骤：

1. **校验一致性**：确保 `eval_set` 与 `infer_set` 长度一致；
2. **加载嵌入模型**：启动 `lazyllm.TrainableModule('bge-large-zh-v1.5')` 用于语义编码；
3. **逐样本评分**：
    - 计算三项指标得分；
    - 保存每条样本的详细对比结果（含上下文、真实答案、模型输出及各分数）至 `infer_true_cp.json`；
4. **汇总统计**：
    - 计算三项指标的平均分（比例）及百分比；
    - 将总体结果以结构化格式写入 `sft_llm_results.jsonl`；
5. **返回可读报告**：打印三项指标的得分与百分比。
...
```
Exact Score : 2.71% -> 23.71%
Cosine Score: 79.23% -> 87.13%
Origin Score: 13.14% -> 34.14%
```

同时生成两个文件：
- `infer_true_cp.json`：包含每条样本的详细对比，便于人工抽查与错误分析；
- `sft_llm_results.jsonl`：结构化存储总体评估结果，支持后续自动化分析或可视化。
---
