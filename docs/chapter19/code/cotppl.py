...
def prepare_dataset():
    tasks = [
        ...
        "logical_deduction_five_objects","logical_deduction_three_objects",
        "multistep_arithmetic_two","object_counting","reasoning_about_colored_objects",
        "temporal_sequences","tracking_shuffled_objects_three_objects",
        "web_of_lies","word_sorting"
    ]
    print("下载lukaemon/bbh 数据集ing.....")
...
def run_cot_pipeline():
    ...
    teacher = lazyllm.TrainableModule(
    ...
    train_data = [{"instruction": x["instruction"], "output": x["output"]} for x in result[:split_idx]]
    test_data  = [{"instruction": x["instruction"], "output": x["output"]} for x in result[split_idx:]]
...
def build_sft_model(model_path):
    return (
        ...
        .prompt(
            dict(system=SYS_PROMPT, drop_builtin_system=True)
        )
...
def score_detailed(path, data):
    stats = {
        "total": 0,
        "standard_format_count": 0,    # 标准格式数量
        "standard_correct": 0,         # 标准格式且对的数量
        "wrong_format_count": 0,       # 错误格式数量
        "wrong_format_correct": 0,     # 错误格式但搜到正确答案的数量
        "parse_fail": 0                # 完全无法提取
    }
    ...
    print(f"  总正确率 (Acc): {total_correct / stats['total'] if stats['total'] else 0:.4%}")
    print("=" * 40)
