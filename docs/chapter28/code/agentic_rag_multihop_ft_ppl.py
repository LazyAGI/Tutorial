...
目标：
  ...
  2. 基于支持文档运行 Agentic RAG 的 atomic / depth / width 流水线
  3. 生成增强样本
  4. 使用"原始样本 + 增强样本"做一次微调
  5. 在同一验证集上对比微调前后的效果
...
使用示例：
  ...
  python agentic_rag_multihop_ft_ppl.py --build_augmented_dataset
  python agentic_rag_multihop_ft_ppl.py --max_train_samples 500
  python agentic_rag_multihop_ft_ppl.py --eval_test
  python agentic_rag_multihop_ft_ppl.py --max_eval_samples 100
"""
# noqa: E402
import argparse
import json
...
# 延迟导入：先设置路径，再导入 lazyllm
def _setup_lazyllm_path():
    """设置 LazyLLM 路径，延迟执行避免 E402"""
...
from lazyllm import LOG, finetune, launchers  # noqa: E402
from lazyllm.tools.data.pipelines.rag_pipelines import (  # noqa: E402
    atomic_rag_pipeline,
    depth_qa_pipeline,
...
def _pred_to_str(x: Any) -> str:
    ...
    if isinstance(x, dict):
        return (x.get('content') or x.get('text')
                or x.get('output') or '').strip()
...
def extract_context_docs(
    example: Dict[str, Any],
    prefer_supporting: bool = True,
) -> List[Dict[str, str]]:
    ...
    for idx, title in enumerate(titles):
        ...
        if not text:
            continue
        if (prefer_supporting and support_titles
                and title not in support_titles):
            continue
...
def build_raw_training_records(
    ...
    for idx, ex in enumerate(train_examples):
        ...
        context = flatten_example_context(
            ex, max_context_chars=max_context_chars, prefer_supporting=True)
        if not question or not answer or not context:
            continue
        rows.append(make_ft_record(context, question,
                    answer, 'raw', f'raw_{idx}'))
...
def build_source_documents(
    ...
    for ex_idx, ex in enumerate(train_examples):
        for doc_idx, doc in enumerate(
                extract_context_docs(ex, prefer_supporting=True)):
...
def generate_atomic_records(
    ...
    for idx, item in enumerate(results):
        ...
        if not context or not question or not answer:
            continue
        ft_rows.append(make_ft_record(context, question,
                       answer, 'atomic', f'atomic_{idx}'))
        width_seed.append(
            {
                'question': question,
...
def generate_width_records(
    ...
    for item in tqdm(results, desc='generate width qa'):
        ...
        answer = answer_helper.answer(
            context=merged_context, question=question, hints=hints)
        if not answer:
            continue
        ft_rows.append(
            make_ft_record(
...
def calculate_eval_metrics(
    ...
    avg_f1 = 0.0
    if scored:
        avg_f1 = sum(float(item.get('F1Score', 0.0) or 0.0)
...
def evaluate_model_on_evalset(
    ...
    if output_path:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            json.dump({'metrics': metrics, 'details': details},
...
示例:
  ...
  python agentic_rag_multihop_ft_ppl.py --max_eval_samples 100
    ...
    flow.add_argument('--run_experiment', action='store_true',
                      help='一键执行：构建原始集 + 增强集 + 基线评测 + 微调 + 复测')
    flow.add_argument('--build_dataset',
                      action='store_true', help='构建原始训练/评测集')
    flow.add_argument('--build_augmented_dataset',
                      action='store_true', help='构建增强训练集')
    ...
    data.add_argument('--dataset_config', type=str,
    ...
    model.add_argument('--finetuned_model_path', type=str,
                       default=None, help='仅评测时可传入已微调模型路径')
...
def main(args: argparse.Namespace) -> None:
    ...
    if not any([args.build_dataset, args.build_augmented_dataset,
                args.train_flag, args.eval_test]):
        print('请指定 --run_experiment / --build_dataset / '
              '--build_augmented_dataset / --train_flag / --eval_test 至少其一')
        return
    ...
    train_examples = []
    eval_examples = []
    if (args.build_dataset or args.build_augmented_dataset
            or args.train_flag or args.eval_test):
    ...
    if args.build_dataset:
        ...
        raw_train_rows = build_raw_training_records(
            train_examples, max_context_chars=args.max_context_chars)
        eval_rows = build_eval_records(
            eval_examples, max_context_chars=args.max_context_chars)
    ...
    if args.build_augmented_dataset:
        ...
        merged_rows = deduplicate_records(
            raw_train_rows + aug_data['all_rows'])
    ...
    if args.train_flag:
        if not os.path.isfile(merged_train_path):
            if raw_train_rows:
                save_jsonl(merged_train_path, raw_train_rows,
                           keep_all_fields=False)
            else:
                print('\n错误：训练数据不存在，请先执行 --build_dataset '
                      '或 --build_augmented_dataset')
                return
