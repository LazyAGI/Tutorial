...
其中 `vocab.json` 记录了模型所能识别的全部 token 及其对应的整数 ID，部分内容示例如下：
...
在实际使用中，现代框架通常通过一个 `tokenizer.json` 文件统一加载上述信息，由框架一次性读取和初始化分词器。这个文件可以被视为“分词器的完整说明书”，它明确规定了模型如何将原始文本稳定地映射为 token ID 序列。
...
- <|endoftext|>词元被分配了一个较大的词元 ID，即 50256。事实上，用于训练 GPT-2、GPT-3 和 ChatGPT 中使用的原始模型的 BPE 分词器的词汇总量为 50 257，这意味着 <|endoftext|>被分配了最大的词元ID。 
