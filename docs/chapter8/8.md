...
下面是一个简单的使用 LazyLLM 实现的 Embedding 使用示例（Qwen 的 text-embedding-v4）：

```python
from lazyllm import OnlineEmbeddingModule

embed = OnlineEmbeddingModule(source='qwen', embed_model_name='text-embedding-v4')

res = embed('我喜欢编程')
print(res)
```

在生成 embedding 之前，模型会先将输入文本进行分词（tokenization），并映射为 token ID。每个 token 对应一个向量表示，最终通过模型内部的聚合机制得到固定维度的句向量。如下：

```bash
[-0.05320797488093376, -0.04214707762002945, 0.010928358882665634, ..., 0.011139214970171452]
```
...
可以看到，text-embedding-v4 向量模型默认输出的是 1024 维向量：
