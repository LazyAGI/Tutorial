...
![预训练语料可分为通用文本与专用文本](./assets/1.png)

在实际预训练中，专用文本通常主要来自三大类资源：

![专用文本主要包括多语文本、科学文本和代码语料](./assets/8.png)
...
![预训练数据清洗流程包括解析、过滤、扩展与去重](./assets/2.png)

### 2.1 质量过滤

质量过滤是整个清洗流程的第一道大关，它的目标很直接：把明显不适合训练的内容筛掉。目前常用两类方法。

![质量过滤方法可分为规则过滤和分类器过滤](./assets/10.png)
...
> 💡 注意：`langdetect.detect()` 并不一定返回 "zh"，而是更细粒度的语言标签。对中文，常见返回值是："zh-cn"（简体中文）、"zh-tw"（繁体中文）、"zh"（较少）。
...
```bash
版权所有 © 2024 某某公司 | 联系我们 True
这是正文内容的一部分，用于模型训练。 False
```
...
![多粒度去重流程包括句子级、文档级和数据集级](./assets/9.png)

推荐的**执行顺序**为从粗到细：**数据集级粗去重 → 文档级去重 → 句子级精细去重**。下文 **（一）～（三）** 按此顺序编排，与典型流水线一致。
...
对 64 位指纹而言，`value >> 16` 相当于**用高位段做粗聚类**：同一桶内更有可能是近重复候选；不同桶则不再两两比较，从而**用分桶换计算量**。代价是可能与「全量两两 SimHash」不完全一致，存在漏判或误判，工程上常配合多段哈希或多表 LSH。桶内仅保留一个代表指纹是教学示意；线上可对每桶维护候选列表或二次校验。SimHash 对文本微小差异不敏感，速度极快，是网页去重中的经典方法。
...
![数据混合与数据课程共同决定训练时的数据调度](./assets/3.png)
...
![数据课程示意为从通用数据逐步过渡到更专业和高质量的数据](./assets/11.png)
...
从模型的视角看，词表并不是传统意义上的“词典”，而是一个**封闭的符号空间**。模型的输入 embedding 层以词表中的每一个 token 作为查表单元，而输出阶段的 softmax 则在整个词表范围内进行概率归一化。换句话说，模型在预训练开始之前，其能够生成的所有最小符号就已经被词表完整定义，模型不可能“发明”新的 token，只能在既有集合中进行选择与组合。无论是自然语言中的词语、标点符号，还是 JSON 的括号、代码里的关键字，本质上都只是这一离散符号空间中的不同编号。
...
- `<|endoftext|>` 词元被分配了一个较大的词元 ID，即 50256。事实上，用于训练 GPT-2、GPT-3 和 ChatGPT 中使用的原始模型的 BPE 分词器的词汇总量为 50,257，这意味着 `<|endoftext|>` 被分配了最大的词元 ID。
- BPE 分词器可以正确地编码和解码未知单词，比如“someunknownPlace”。
...
![BPE 将未知词拆分为子词或字符的示意图](./assets/7.png)
...
$$
\text{Params}_{\text{vocab}} \approx |V| \times d_{\text{model}}
\;(\text{embed})
\;+\;
|V| \times d_{\text{model}}
\;(\text{lm\_head})
...
若采用 **weight tying**（输出层与输入 embedding 共用同一套 $|V| \times d_{\text{model}}$ 权重），则扩表在参数上主要仍体现为**这一套矩阵随 $|V|$ 变大**，但前向中 softmax 仍在整词表上计算，算力与显存占用仍会随 $|V|$ 上升。
...
- [Transformer 学习笔记](https://transformers.run/c2/2021-12-11-transformers-note-2/)
- [BPE（Byte Pair Encoding）原理与实现](https://carota.site/posts/tech/bpe)
