...
然后计算去除空白后的字符级 2-gram 重合指标。它的形式更接近 Precision / Recall / F1 的组合分数，而不是标准 BLEU：
...
这里使用的是便于快速比较的简化指标，适合同一实验设置下的相对对比，但不能等同于标准 BLEU 分数。
...
!!! note "实验备注（待补充）"
    当前配套脚本的评测流程仍有两个待完善点，因此本节表格和截图更适合作为流程示意，不宜直接视为严格可比的最终实验结论。

    1. 当前脚本中的训练集与评测集切分方式仍需改为严格互斥，否则会带来训练/评测重叠风险。
    2. 当前训练样本采用 `Question: ...\nAnswer: ...` 模板，而评测输入口径仍需进一步与训练模板对齐，以便更准确评估 QA 续写能力。

    后续建议在完成严格数据切分、统一评测模板后，重新生成本节结果表与运行截图。

---

## 5 本章小结

本章围绕 LazyLLM，展示了构建完整 LLM 实践链路的全过程。首先，在**数据流**方面，我们通过数据合成将原始文本转化为结构化的 Phi-4 风格问答数据，从“无结构数据”变为可直接用于模型训练的语义结构数据，本质上是**利用大模型生成训练数据（Data Synthesis）**。其次，在**模型流**方面，同一框架完成了训练、评测与部署，有效降低了工程复杂度。再者，在**评测体系**上，我们结合多指标进行衡量：PPL 用于评估语言建模能力，Loss 衡量概率拟合程度，2-gram F1（BLEU 风格重合指标）用于检查生成结果与参考答案的局部重合质量。最核心的思想在于：**数据质量不等于原始数据本身的质量，而在于数据的构造方式**。通过 Phi-4 风格数据合成，我们不仅提升了语义建模能力，也使训练方式更贴近实际应用，同时加快了模型迭代效率。本章完成了一个完整的**数据 → 模型 → 应用**闭环流程，并为后续扩展提供了基础，例如引入 RAG（检索增强）、多语言数据合成或多模态数据扩展，从而构建更复杂、更实用的大模型系统。
