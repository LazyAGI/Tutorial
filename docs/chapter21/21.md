...
!!! note "实验备注（待完善）"
    当前脚本中的评测集并不是与训练集严格互斥的独立切分。因为代码先执行 `train_list = rows[:TRAIN_SAMPLES]`，随后又从 `train_list` 内部抽样生成 `eval_list`，所以评测样本实际来自训练候选集内部。

    这意味着本章结果更适合作为“长上下文 pipeline + LoRA-SFT 跑通”的示意，而不应直接视为严格 benchmark。若后续需要更可靠的实验结论，建议改为互斥切分，例如：

    - 训练集：`rows[:TRAIN_SAMPLES]`
    - 评测集：`rows[TRAIN_SAMPLES:TRAIN_SAMPLES + EVAL_SAMPLES]`

    完成切分修正后，建议重新生成 `squad_ppl_train.json` / `squad_ppl_eval.jsonl`，并重跑 5.7 节结果表与截图。
