...
| 参数 | 值 | 说明 |
| :--- | :--- | :--- |
| `learning_rate` | `5e-5` | 小模型（0.5B）适当调小，避免 Loss 震荡 |
| `cutoff_len` | `4096` | 覆盖含工具定义的完整 System Prompt |
| `val_size` | `0.1` | 10000 条数据中取 10% 作验证集 |
| `num_train_epochs` | `3.0` | 工具调用任务通常 2~3 轮即可收敛 |
| `template` | `qwen` | 匹配 Qwen 系列模型的对话模板 |
...
$$\text{Refusal Success Rate} = \frac{1}{n}\sum_{i=1}^{n} \mathbb{1}\bigl[\hat{r}_i = 1 \mid r_i^* = 1\bigr]$$
