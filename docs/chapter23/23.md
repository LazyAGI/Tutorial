...
3. **数据处理实战**：如何用 Agent 智能解析文档、生成摘要、注入元数据；
...
- 对每个块生成简短摘要：
  ...
  $$
    ...
    - $d_i$：第 $i$ 个原始文本块；
    - $\text{LLM}_{\text{sum}}$：用于摘要的 LLM（如 LLaMA-3-8B）；
    - $s_i$：生成的摘要文本。
...
![image.png](../assets/23.5.png)

文件中的内容大致如下：

![image-2.png](../assets/23.6.png)


---

## 5. RAG实战：基于LazyLLM实现基础RAG
