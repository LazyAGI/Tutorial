===
1. 代码使用flake8进行检查（待修改）   
2. 文档末尾不要有多余空行（仅保留一行，已修改）  
3. 仔细检查确保每个公式的变量都有明确解释  
4. 3.7 不要作为单独的一节  
5. 1.2.1 节没必要作为单独的一节，1.2只有这一个子小节（3.3.1也是如此）  
6. 3.2.0节？？？（从3.2.1开始）  
7. 代码部分需要增加一些必要的文字描述  
===
...
#### 3.2.1 Step 1：图片保存与预处理

一键启动脚本会先将数据集中的 `PIL.Image` 保存为本地图片文件，再在后续 `img2qa` 流水线中统一做图像缩放。
...
👉 算子：`build_img2qa_pipeline`  
根据图片和问题生成带推理过程的增强答案，并直接写回训练集。
...
#### 3.2.3 Step 3：直接用于 SFT 训练

一键启动脚本不会额外做质量评分过滤，也不会单独再执行一次格式转换，而是直接基于训练集中的以下字段进入 VLM 微调：
...
本实验属于小规模数据（1793 条 train）微调场景，因此训练策略以稳定为主。脚本采用 `learning_rate=1e-5`，用于降低对预训练能力的破坏；采用 `num_train_epochs=3.0`，让模型在有限数据上完成充分适应；同时使用 `gradient_accumulation_steps=10`，在 `per_device_train_batch_size=12` 的基础上得到约 `12 × 10 = 120` 的等效 batch size，从而提升梯度稳定性、减少小数据训练带来的波动。整体来看，这是一套偏保守但更稳妥的小数据 VLM 微调配置。
