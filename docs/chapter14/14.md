...
ScienceQA 是多模态 Chain-of-Thought 的开山之作。它不仅仅给出“图像+问题+答案”，还额外提供了**知识背景**和**推理过程**。
...
数据集中包含了三个层面的信息：
  ...
  * ​**感知层 (`image`, `question`, `choices`)**​：模型需要“看到”地图，并识别出四个州的位置。
  * ​**知识层 (`lecture`)**​：这是 **多模态 CoT** 的关键。它为模型提供了“外部常识”（例如：北在地图上方）。如果没有这个字段，模型只能靠死记硬背；有了它，模型学会的是​**逻辑迁移**​。
  * ​**逻辑层 (`solution`)**​：这是显式的推理路径。它告诉模型：第一步看指北针，第二步对比位置，第三步得出结论。
...
虽然上面数据看起来像是一个普通的字典，但它的“交错”属性隐藏在 **`images`** 和 **`texts`** 这两个列表的**索引对应关系**中。

在多模态预训练中，这种格式被称为 ​**基于索引的交错（Index-based Interleaving）**​。

#### 2.2.1 结构化解析

OBELICS 并不直接存储一个长字符串，而是通过**列表对齐**的方式来还原网页的原始布局：

| **索引 (Index)** | **images 列表内容** | **texts 列表内容**   | **还原后的序列**              | 
| ------------------------ | --------------------------- | ---------------------------- | ------------------------------------- |
| **0**                 | `https://cdn...jpg`   | `None`                 | **[图像]**(兰博基尼图片)           |
| **1**                 | `None`                | `'The buyer would...'` | **[文本]**"The buyer would get..." |

* ​**逻辑原理**​：当 `images[i]` 有值时，`texts[i]` 通常为 `None`；反之亦然。模型在读取数据时，会将这两个列表按顺序“拉链式”合并。
* ​**元数据锚点**​：`metadata` 字段中的 `alt_text`（如 "VW Group Allegedly Receives Offer..."）为图像提供了强相关的上下文描述，这对模型理解图像在文中的语义作用至关重要。



#### 2.2.3 模型看到的“交错”视图

在进入模型（如 IDEFICS 或 Flamingo）训练前，这段 JSON 会被预处理成类似下面的​**流式序列**​：

```
<IMAGE> [兰博基尼 Urus 碳纤维配件图] </IMAGE><TEXT> The buyer would get... </TEXT>
```

#### 2.2.4 核心特点

* ​**原生交错（Native Interleaving）**​：与 ScienceQA 这种人工构造的“一问一答”不同，OBELICS 来源于对 Common Crawl 网页的解析。它刻画了人类最真实的阅读习惯——​**通过图像引入话题，随后展开深度文字论述**​。
* ​**超大规模**​：包含 1.15 亿个间歇插入图像的文本文档，总量达 3.53 亿张图像，是训练多模态大模型（LMM）理解长上下文的基础。
* ​**语义一致性**​：`texts[1]` 提到的 “Quantum Group AG” 收购案，其背景直接映射到 `images[0]` 所展示的品牌，模型借此学习​**跨模态指代消解**​（即知道文中的“Lambo”就是图中的车）。
