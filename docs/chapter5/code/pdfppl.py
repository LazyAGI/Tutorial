...
'''
    ...
    print(f"生成数据完成: Total={len(results)}, Train={len(train_data)}, Test={len(test_data)}")
...
def build_infer_model(model_path):
    model = lazyllm.TrainableModule(model_path).prompt(dict(system="", drop_builtin_system=True))
    return model
...
def main():
    ...
    eval_prompts = [item["instruction"] + f'上下文：{item["input"]}' for item in test_data]
