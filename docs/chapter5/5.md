...
- （重）计划并分解任务；
- 调用工具解决当前子任务；
- 观察工具输出，确定是否完成子任务，完成整个任务就结束循环 | - 计划并分解任务；
- 调用工具逐步解决所有子任务
- 综合所有步骤结果进行反馈 |
...
为此，我们设计了一个 ​**基于 ReAct 框架的数据质量修正 Agent**​，用于在数据进入微调阶段前，对 QA 样本进行​**自动评估与修正**​。
...
![](./assets/agentResult.png)

---

### 2. Agent 架构设计

本实战采用 LazyLLM 提供的 **ReActAgent** 作为控制器，其核心组成包括：

#### （1）工具定义（Tools）

* `evaluate_row`
  用于判断给定 QA 是否与 Content 语义一致，输出布尔值。
* `regenerate_row`
  在 QA 不一致时，根据 Content 重新生成新的 Query–Answer 对。

工具通过 `@fc_register("tool")` 注册，使 Agent 能够在推理过程中动态调用。

#### （2）Agent 执行逻辑

Agent 的推理策略被显式写入 Prompt 中：

1. 先调用 `evaluate_row`
2. 若返回 `False`，调用 `regenerate_row`
3. 对新生成的 QA 再次进行评估
4. 直到 QA 通过校验或达到最大重试次数

这是一种典型的 **Evaluation-driven Agent** 结构。
...
这一过程完整体现了 ReAct 框架中的 **Thought → Action → Observation → Feedback** 机制。

---

### 4. 方法优势

相比传统的静态数据清洗规则，该 Agent 方案具有以下优势：

* ​**自适应修正**​：不依赖固定规则，适用于多领域 QA 数据；
* ​**可扩展性强**​：可轻松接入 BLEU、ROUGE、Embedding 相似度等自动指标；
* ​**可解释性好**​：每一步决策都通过 ReAct 日志显式记录；
* ​**高度自动化**​：适合大规模合成数据与微调数据生产流水线。
