...
可生成包含图片路径的对话格式数据，用于视觉语言模型（VLM）训练。或者纯本文微调的Alpaca训练格式。
...
output_folder = '/xxxxxxxxxxx/experiment/pdf_experiment/pdfs/images'
model = lazyllm.TrainableModule('Qwen2.5-VL-32B-Instruct').deploy_method(
    lazyllm.deploy.vllm,
    url='http://10.119.17.248:38334/v1/'
    
)
data = []

for p in pdf_folder.glob("*.pdf"):
    data.append({pdf_input_key: str(p)})

print(f"找到 {len(data)} 个 PDF")
