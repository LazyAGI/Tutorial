...
## 附录：Text2SQL [一键启动脚本](./code/run_text2sql_pipeline.sh)使用说明

本章节配套的 `run_text2sql_pipeline.sh` 脚本提供了从数据转换到模型评估的完整自动化流程。
...
| 占位符 | 说明 | 示例 |
|--------|------|------|
| `<OUTPUT_BASE_DIR>` | 输出基础目录 | `/path/to/output` |
| `<LAZYLLM_PATH>` | LazyLLM 安装路径 | `/path/to/lazyllm` |
| `<PIPELINE_MODEL>` | Pipeline 使用的模型路径 | `/path/to/qwen-30b` |
| `<SFT_BASE_MODEL>` | SFT 训练基础模型路径 | `/path/to/qwen-0.5b` |
| `<JUDGE_MODEL>` | 评估评判模型路径 | `/path/to/qwen-30b` |
| `<LOCAL_PYTHON_PACKAGES_PATH>` | Python site-packages 路径 | `/path/to/python3.10/site-packages` |

### 2. 运行方式

```bash
# 运行完整流程（数据转换 -> Pipeline生成 -> 训练 -> 测试 -> 评估）
bash run_text2sql_pipeline.sh

# 仅运行单个步骤
bash run_text2sql_pipeline.sh transform    # 数据转换
bash run_text2sql_pipeline.sh pipeline     # Pipeline 生成
bash run_text2sql_pipeline.sh train        # 模型训练
bash run_text2sql_pipeline.sh test         # 模型测试
bash run_text2sql_pipeline.sh eval         # 结果评估
```
...
# 限制 Pipeline 处理数据量（默认1000条）
bash run_text2sql_pipeline.sh all -m 500

# Dry-run 模式：只显示将要执行的命令，不实际运行
...
| 步骤 | 功能 | 输入 | 输出 |
|------|------|------|------|
| [1/5] 数据转换 | 将原始 JSON 转换为 pipeline 所需格式 | `hardest_5000_sql_sft.json` | `hard_sql.json` |
| [2/5] Pipeline | 使用 LLM 生成合成训练数据 | `hard_sql.json` | `ppl_sql.json` |
| [3/5] 模型训练 | 基于生成的数据进行 SFT 微调 | `improved_ppl_sql.json` | `enhance_ppl_checkpoint/` |
| [4/5] 模型测试 | 使用训练好的模型进行推理测试 | 测试数据 | `enhance_ppl.json` |
| [5/5] 结果评估 | 使用 LLM as Judge 评估生成质量 | 预测结果 + 参考答案 | 评估报告 |

### 5. 注意事项

- **不要用 `source` 执行此脚本**，直接使用 `bash` 运行
- 确保所有路径配置正确且可访问
- 训练步骤需要 GPU 资源，请确保环境可用
- 日志文件自动保存在 `logs/` 目录下，命名格式为 `pipeline_YYYYMMDD_HHMMSS.log`
- 脚本运行过程中会生成临时 Python 文件，正常结束后会自动清理