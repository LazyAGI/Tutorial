...
```python
# run_cmeie.py prepare_dataset() 核心逻辑
...
| 逻辑单元 | 输入 | 输出 | 功能 |
|---|---|---|---|
| `_format_cmeie_to_sft_single(item)` | CMeIE 单条（`text`, `spo_list`） | SFT 样本 dict / `None` | 提取首条 `spo`，解析 `object`（含 `@value`）并组织为单三元组输出。 |
| `_is_valid_single(entry)` | 单条 SFT 样本 | `bool` | 校验 `input` 非空且长度 `<=512`，`output` 为可解析且字段完整的 JSON。 |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| 基座模型 | 0.005 | 0.217 | 1.00 | 0.00 | 0.005 | 1.00 | 0.010 |
| 微调模型 | 0.215 | 0.535 | 1.00 | 0.00 | 0.215 | 1.00 | 0.354 |

结论：从基座到微调模型，`exact_match`（0.005→0.215）、`slot_accuracy`（0.217→0.535）以及 `strict_precision/strict_f1`（0.005/0.010→0.215/0.354）均明显提升，说明 SFT 让模型不仅“会按格式输出”，而且在三元组语义层面也更接近标注答案。进一步看，`json_compliance=1.00` 与 `empty_pred_rate=0.00` 在两组实验中都保持稳定，表明本流程在输出可解析性与推理稳定性上已有较好工程可靠性。综合来看，本次实践验证了“单三元组格式约束 + LoRA 监督微调”对医疗结构化抽取任务是有效路径；后续可通过增加训练数据覆盖、引入多三元组样本与关系类型均衡采样，继续提升 `exact_match` 与 `strict_f1` 上限。
