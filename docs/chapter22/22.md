...
* 统计分布 (Loss Value Distribution)：大部分 Loss 集中在 0.65-0.7 之间，表明模型在测试集上的表现高度一致，具备良好的泛化能力。
...
1.计算 TD Target & Delta:$TD\_Target = r_t + \gamma \times V(s_{t+1})$
...
* **🔵 Advantage: GAE (通用优势估算) —— [蓝色线]**
    ...
    * **原理**: GAE (Generalized Advantage Estimation) 是 PPO 论文中推荐的标准做法。它通过一个超参数 $\lambda$ 平衡了方差和偏差。图中显示它结合了 MC 的准确性和 TD 的稳定性，是**最通用的选择**。
...
​**kl​ (KL 散度)**​: 这个值不能太大（通常在 0.1 - 2.0 之间）。如果突然变成几十甚至几百，说明模型“崩了”（为了拿分开始乱说话），需要调低 learning_rate。
