...
| 数据集名称 | 核心特点与贡献 | 数据来源 | 规模 (约) | 典型应用场景 |
| :--- | :--- | :--- | :--- | :--- |
| **ToolBench** | **广度优先**：涵盖 16k+ 真实 API，采用 DFS 算法构建多样的推理路径，强调泛化能力。 | RapidAPI (真实接口) | 126k+ 轨迹 | 通用工具调用、复杂多步指令跟随 |
| **Gorilla** | **准确性优先**：专注于减少 API 幻觉，包含 TorchHub, HuggingFace 模型调用，强调参数匹配的精确度。 | 文档解析 + Self-Instruct | 16k+ 样本 | 代码生成、ML 模型调用、零样本 API 检索 |
| **API-Bank** | **评测基准**：首个具备完整评估系统的基准，包含工具使用的三个层级（调用、检索、管理）。 | 人工标注 + 合成 | 73 种 API | 评估模型在对话系统中调用 API 的准确率 |
| **AgentInstruct** | **高质量合成**：利用 GPT-4 生成的高质量合成数据，覆盖操作系统、数据库等交互环境，强调长序列规划。 | 合成数据 (Synthetic) | 1.8k+ 核心种子泛化 | 操作系统控制、文件处理、复杂逻辑任务 |
| **Mind2Web** | **Web 代理**：专注于真实网页操作的数据集，记录了从 DOM 元素定位到点击/输入的完整过程。 | 真实网页标注 | 2k+ 任务 | 浏览器自动化 Agent、网页导航 |
...
**核心理念**：Simia-SFT 和 Simia-RL 提出了一种“环境无关”的数据生成范式。其核心在于训练一个专门的**“模拟器 LLM (Simulator LLM)”**，它不执行任务，而是根据Agent的动作，预测环境应有的反应。
...
#### 5.1.2 边缘情况与错误注入 (Robustness Training)
...
def prepare_training_data(example, tokenizer):
    ...
    # 注意：这里可能需要根据具体 Tokenizer 处理 bos_token 或 offset
...
* **多样化停止条件**：在训练数据中明确标记结束符（如 `<|eot_id|>` 或特殊字符串 `Final Answer:`），并在 Loss 计算中对该 Token 进行加权。

* **DPO/RLHF 惩罚**：

    * 构建包含死循环的负样本对 $(y_w, y_l)$，其中 $y_l$ 包含重复步骤。

    * 设置 **Repetition Penalty (重复惩罚)**：如果检测到连续 $N$ 次相同的 Action，给予极大的负 Reward。
