...
结合 lazyllm.tools.data.pipelines.reranker_pipelines 与 operators.reranker_synthesis，
支持两种输入方式并实现自动化流程。

功能：
1. 两种输入：① 用户输入数据（本地 JSON/JSONL） ② HuggingFace 数据集名（自动下载）
2. 数据准备与 Pipeline：划分、难负样本挖掘、格式化（flagreranker / cross_encoder / pairwise）
...
from lazyllm.tools.data.operators.reranker_synthesis import RerankerTrainTestSplitter
...
def load_from_user_data(
    ...
    LOG.info(f"从用户数据加载: {data_path} -> {len(raw_items)} 条，语料 {len(corpus_texts)} 篇")
...
def load_from_huggingface(
    ...
    q_col = query_column if query_column in cols else ("query" if "query" in cols else "question")
    p_col = pos_column if pos_column in cols else ("pos" if "pos" in cols else "context")
    ...
    LOG.info(f"从 HuggingFace 加载: {dataset_name} -> {len(raw_items)} 条，语料 {len(corpus_texts)} 篇")
    return raw_items, corpus_texts
...
def load_from_fiqa(
    ...
    """从 FiQA 文件加载并提取 (query, pos) 列表及语料。仅做提取，划分/难负样本/保存由 pipeline 完成。"""
    queries, corpus, train_pairs = _extract_raw_pairs_from_fiqa(queries_file, corpus_file, train_file)
    raw_items = []
...
def build_rerank_dataset_from_embed_data(
    ...
    flat = [x for item in converted for x in (item if isinstance(item, list) else [item])]
...
def deploy_reranker_model(
    ...
    """部署带微调能力的 Reranker。"""
    print(f"\n{'='*60}")
    print(f"配置 Reranker: {rerank_path}，微调: {'开启' if train_flag else '关闭'}")
    print(f"{'='*60}\n")
...
def evaluate_reranker_direct(
    ...
    for item in tqdm(eval_data, desc="评估"):
        ...
        if not all_docs:
            continue
        results = reranker_model(query, documents=all_docs, top_n=len(all_docs))
...
def evaluate_reranker_with_retriever(
    ...
    for item in tqdm(eval_data, desc="检索+重排"):
        ...
        results.append({
            ...
            "context_reference": item.get("corpus", item.get("pos", [])),
        })
...
def save_results(score_dict: Dict[str, Any], train_flag: bool, file_path: str) -> None:
    """追加保存评估结果。"""
    prefix = "ft_" if train_flag else "origin_"
    with open(file_path, "a", encoding="utf-8") as f:
        f.write(json.dumps({f"{prefix}{k}": v for k, v in score_dict.items()}, ensure_ascii=False) + "\n")
...
示例:
  ...
  输入方式二 - HuggingFace:
    python Reranker_ft_ppl.py --build_dataset --dataset_name virattt/financial-qa-10K
  从 Embedding 数据转换:
    python Reranker_ft_ppl.py --build_dataset --from_embed --embed_train_path dataset/embed_train.json
  微调:
    python Reranker_ft_ppl.py --train_flag --rerank_path BAAI/bge-reranker-base --num_epochs 1
        """,
    )

    data_group = parser.add_argument_group("数据参数")
    data_group.add_argument("--build_dataset", action="store_true", help="构建数据集")
    data_group.add_argument("--data_path", type=str, default=None, help="输入方式一：本地 JSON/JSONL 路径")
    data_group.add_argument("--query_key", type=str, default="query")
    data_group.add_argument("--pos_key", type=str, default="pos")
    data_group.add_argument("--use_fiqa", action="store_true", help="使用本地 FiQA")
    data_group.add_argument("--dataset_name", type=str, default="virattt/financial-qa-10K", help="输入方式二：HuggingFace 数据集名")
    ...
    data_group.add_argument("--from_embed", action="store_true", help="从 Embedding 训练数据转换")
    ...
    data_group.add_argument("--queries_file", type=str, default="dataset/fiqa/queries.jsonl")
    data_group.add_argument("--corpus_file", type=str, default="dataset/fiqa/cpt/corpus.jsonl")
    data_group.add_argument("--train_file", type=str, default="dataset/fiqa/train.tsv")
    ...
    data_group.add_argument("--output_subdir", type=str, default="/home/mnt/zhangkejun/work/dataset/reranker_ft")

    ppl_group = parser.add_argument_group("Pipeline 参数")
    ppl_group.add_argument("--mining_strategy", type=str, default="random", choices=["random", "bm25", "semantic", "mixed"])
    ppl_group.add_argument("--output_format", type=str, default="flagreranker", choices=["flagreranker", "cross_encoder", "pairwise"])
    ppl_group.add_argument("--train_group_size", type=int, default=8)

    model_group = parser.add_argument_group("模型参数")
    model_group.add_argument("--rerank_path", type=str, default="BAAI/bge-reranker-base")
    model_group.add_argument("--embed_path", type=str, default="BAAI/bge-small-zh-v1.5")
    ...
    eval_group.add_argument("--output_path", type=str, default="/home/mnt/zhangkejun/work/dataset/reranker_ft/rerank_eval_results.jsonl")
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.build_dataset:
        ...
        if args.from_embed:
            embed_train = args.embed_train_path or build_data_path("dataset", "embed_train.json")
            embed_eval = args.embed_eval_path or build_data_path("dataset", "embed_eval.json")
            train_data_path, eval_data_path = build_rerank_dataset_from_embed_data(
                embed_train, embed_eval, args.neg_num, args.output_subdir
            )
        else:
            if args.data_path:
            ...
            train_data_path, eval_data_path, kb_path = build_dataset_with_pipelines(
                raw_items=raw_items,
    ...
    if not os.path.exists(train_path):
        print("未找到 rerank_train.jsonl，请先执行 --build_dataset [--data_path PATH | --use_fiqa | --dataset_name NAME]")
        return
    ...
    if args.eval_with_retriever:
        ...
        if not os.path.exists(kb_path):
            ...
            docs.create_node_group("split_sent", transform=lambda s: s.split("\n"), metadata=lambda _: {"source": "kb"})
            retriever = Retriever(doc=docs, group_name="split_sent", similarity="cosine", topk=args.retriever_topk)
            reranker_wrapper = Reranker(name="ModuleReranker", model=reranker_model, topk=args.topk)
            retriever.start()
            _, recall_score, relevance_score = evaluate_reranker_with_retriever(
                retriever, reranker_wrapper, eval_data_path
            )
            save_results({"recall": recall_score, "relevance": relevance_score}, args.train_flag, args.output_path)
            print(f"Context Recall: {recall_score:.4f}, Context Relevance: {relevance_score:.4f}")
    else:
        mrr, hit_rate = evaluate_reranker_direct(reranker_model, eval_data_path, args.topk)
        save_results({"mrr": mrr, f"hit@{args.topk}": hit_rate}, args.train_flag, args.output_path)
