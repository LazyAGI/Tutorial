...
import lazyllm
from lazyllm import Document, Retriever, Reranker, finetune, launchers, TrainableModule
...
def build_rerank_dataset_from_fiqa(
    ...
    for query_id, pos_corpus_ids in train_pairs.items():
        ...
        while len(neg_texts) < neg_num and attempts < neg_num * 10:
            neg_id = corpus_ids[np.random.randint(0, len(corpus_ids))]
            if neg_id not in pos_corpus_ids and corpus[neg_id] not in neg_texts:
...
def build_rerank_dataset_from_huggingface(
    ...
    eval_data_path = build_data_path('dataset', 'rerank_eval.jsonl')
    with open(eval_data_path, 'w', encoding='utf-8') as f:
        for idx, item in enumerate(split["test"]):
            # 为评估数据添加负样本
            neg_indices = np.random.randint(0, len(test_corpus_list), size=neg_num)
            neg = [test_corpus_list[i] for i in neg_indices if test_corpus_list[i] != item["pos"][0]][:neg_num]
            f.write(json.dumps({
                "query": item["query"], 
                "corpus": item["pos"],
                "neg": neg
    ...
    print(f"数据集构建完成：训练样本 {len(train_data)}，测试样本 {len(split['test'])}，知识库语料 {len(test_corpus_list)}")
    return train_data_path, eval_data_path, os.path.dirname(kb_data_path)
...
def deploy_reranker_model(
    ...
    """
    print(f"\n{'='*60}")
    print(f"配置Reranker模型: {rerank_path}")
...
使用示例:
 ...
 微调Reranker模型:
   python reranker_finetune.py --train_flag --rerank_path BAAI/bge-reranker-base --num_epochs 1 --ngpus 1

 使用HuggingFace数据集:
   python reranker_finetune.py --build_dataset --dataset_name virattt/financial-qa-10K
        """
    )
    ...
    model_group.add_argument('--rerank_path', type=str, default='BAAI/bge-reranker-base',
                            help='Reranker模型路径/名称（如BAAI/bge-reranker-base, BAAI/bge-reranker-large）')
    model_group.add_argument('--embed_path', type=str, default='BAAI/bge-small-zh-v1.5',
                            help='Embedding模型路径（用于检索+重排流程）')
    ...
    eval_group.add_argument('--retriever_topk', type=int, default=10,
                           help='检索器返回的候选数量')
    eval_group.add_argument('--output_path', type=str, default='rerank_eval_results.jsonl',
                           help='评估结果保存路径')
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.build_dataset:
        ...
        if args.use_fiqa:
            ...
            train_data_path, eval_data_path, kb_path = build_rerank_dataset_from_huggingface(
    ...
    if not all(os.path.exists(f) for f in [train_path, eval_path]):
        ...
        print("  python reranker_finetune.py --build_dataset --from_embed  (从embedding数据转换)")
        print("  python reranker_finetune.py --build_dataset --use_fiqa    (使用FiQA数据)")
        print("  python reranker_finetune.py --build_dataset               (使用HuggingFace数据)")
        return
    ...
    if args.eval_with_retriever:
        ...
        if not os.path.exists(kb_path):
            ...
            results, recall_score, relevance_score = evaluate_reranker_with_retriever(
                retriever=retriever,
                reranker=reranker,
