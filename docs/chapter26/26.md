...
首先展示如何通过代码构建一个Embedding微调所需的数据集，这里我们使用金融问答数据集：[virattt/financial-qa-10K](https://huggingface.co/datasets/virattt/financial-qa-10K)来进行演示：
...
| 算子名称 | 功能描述 | 支持模式/特性 |
| :--- | :--- | :--- |
| **EmbeddingTrainTestSplitter**<br>*(数据集切分)* | 对输入数据进行随机打乱，并按指定比例分割为训练集和测试集。 | • 支持自定义分割比例<br>• 支持保存分割后的数据到 JSONL 文件 |
| **embedding_data_augmentor**<br>*(数据增强)* | 增加样本多样性，防止模型过拟合，提升泛化能力。 | • **LLM 查询重写**：通过语义改写生成多样化 query 变体<br>• **规则相邻词交换**：基于规则的简单增强 |
| **embedding_hard_negative_miner**<br>*(难负样本挖掘)* | 挖掘高难度负样本（Hard Negatives），提升模型区分能力。 | • **BM25**：基于关键词匹配<br>• **语义相似度**：基于向量距离<br>• **混合方式**：结合上述两种途径 |
| **embedding_data_formatter**<br>*(数据格式化)* | 将处理后的数据转换为主流框架所需的训练格式。 | • **FlagEmbedding** 训练格式<br>• **SentenceTransformers** 三元组训练格式<br>• **通用三元组** 格式 |

这里我们简单演示一下使用 embedding_pipelines 完成：划分 → [增强] → 难负样本挖掘 → 格式化 → 保存。完整代码([embedding_ft_ppl](from-data-to-llm/docs/chapter26/code/Embedding_ft_ppl.py))
...
- [Regional green innovation efficiency and dynamic evolution of Chinese industrial enterprises: a three-stage super-efficiency DEA method based on cooperative game](https://link.springer.com/article/10.1007/s11356-022-21682-0)