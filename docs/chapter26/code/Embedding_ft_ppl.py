...
功能：
1. 两种输入：① 用户输入数据（本地 JSON/JSONL） ② 用户输入 HuggingFace 数据集名（自动下载）
...
from lazyllm.tools.data.operators.embedding_synthesis import EmbeddingTrainTestSplitter
...
def save_json(score_recall, score_relevance, train_flag: bool, file_path: str) -> None:
    """保存评估分数到JSON文件"""
    with open(file_path, 'a', encoding='utf-8') as f:
        if train_flag:
            json.dump({'ft_score_recall': score_recall, 'ft_score_relevance': score_relevance},
                      f, ensure_ascii=False, indent=2)
        else:
            json.dump({'origin_score_recall': score_recall, 'origin_score_relevance': score_relevance},
                      f, ensure_ascii=False, indent=2)
...
def load_from_fiqa(
    ...
    for query_id, pos_corpus_ids in train_pairs.items():
        ...
        if pos_texts:
            raw_items.append({"query": queries[query_id], "pos": pos_texts})
    corpus_texts = list(corpus.values())
    LOG.info(f"从 FiQA 加载: {len(raw_items)} 条 (query,pos)，语料 {len(corpus_texts)} 篇")
...
def load_from_user_data(
    ...
    LOG.info(f"从用户数据加载: {data_path} -> {len(raw_items)} 条 (query,pos)，语料 {len(corpus_texts)} 篇")
    return raw_items, corpus_texts
...
def build_dataset_with_pipelines(
    ...
    调用方负责先通过 load_from_fiqa / load_from_user_data / load_from_huggingface 得到 raw_items（及可选 corpus_texts），
...
def evaluate_retriever(
    ...
    def _extract_passage_from_retrieved(raw_text: str) -> List[str]:
        """若检索结果为整条 JSON（含 query/corpus），只取 corpus 段落用于与 reference 对比。"""
...
输入方式二 - HuggingFace 数据集名:
   python Embedding_ft_ppl.py --build_dataset --dataset_name virattt/financial-qa-10K

本地 FiQA:
   python Embedding_ft_ppl.py --build_dataset --use_fiqa --mining_strategy bm25

微调与评估:
   python Embedding_ft_ppl.py --train_flag --embed_path BAAI/bge-large-en-v1.5 --num_epochs 2
        """,
    )
    ...
    data_group.add_argument('--data_path', type=str, default=None,
                            help='输入方式一：本地数据文件路径（JSON/JSONL），每行或整体为 {"query","pos"} 或可配置键名')
    ...
    data_group.add_argument('--use_fiqa', action='store_true',
                            help='使用本地 FiQA 数据集（与 --queries_file/corpus_file/train_file 配合）')
    data_group.add_argument('--dataset_name', type=str, default='virattt/financial-qa-10K',
    ...
    ppl_group.add_argument('--augment_methods', type=str, nargs='*', default=['query_rewrite'],
                           help='数据增强方法列表 (可选: query_rewrite, synonym_replace)')
    ppl_group.add_argument('--num_augments', type=int, default=1,
                           help='每种增强方法的增强数量')
    ppl_group.add_argument('--augment_lang', type=str, default='en',
                           help='增强语言')
    ppl_group.add_argument('--output_format', type=str, default='flagembedding',
                           choices=['flagembedding', 'triplet', 'sentence_transformers'],
                           help='训练数据输出格式')
    ppl_group.add_argument('--mining_language', type=str, default='en',
                           help='BM25 挖掘语言')
    ppl_group.add_argument('--output_dir', type=str, default='/home/mnt/zhangkejun/work/dataset/embed_ft',
                           help='输出路径')
    # 模型相关参数
    model_group = parser.add_argument_group('模型参数')
    model_group.add_argument('--embed_path', type=str, default='BAAI/bge-large-zh-v1.5',
    ...
    train_group.add_argument('--per_device_batch_size', type=int, default=16,
                             help='每个设备的训练批次大小')
    ...
    eval_group.add_argument('--instruction', type=str,
                            default="Represent this sentence for searching relevant passages: ",
                            help='查询指令模板')
    eval_group.add_argument('--use_instruction', action='store_true',
                            help='在查询前添加指令')
    eval_group.add_argument('--output_path', type=str, default='/home/mnt/zhangkejun/work/dataset/embed_ft/embed_eval_results.jsonl',
                            help='评估结果保存路径')
...
def main(args: argparse.Namespace) -> None:
    ...
    print("=" * 80 + "\n")
    llm = lazyllm.TrainableModule('/mnt/lustre/share_data/lazyllm/models/qwen2.5-14b-instruct')
...
# )
    ...
    if not all(os.path.exists(f) for f in [train_path, eval_path, kb_file]):
        print("\n错误: 未找到处理好的数据集！")
        print("请先运行: python Embedding_ft_ppl.py --build_dataset [--data_path PATH | --use_fiqa | --dataset_name NAME]")
        return
    ...
    # ====== 3. 评估性能 ======
    print("\n>>> 步骤 3: 评估检索性能")
    out_dir = os.path.dirname(os.path.abspath(args.output_path)) or args.output_dir
